<template>
   <CpnavbarVue title="我的处方" />
	<div class="page_liebiao1">
		<div class="yonghu" v-for="(item,index) in recordList" :key="index" @click="onDetail(item.registrationId)">
            <div class="shijian">就诊时间：<span style="margin-left:10px">{{item.createTime}}</span></div>
			<div>
				<div class="name">就诊科室：<span style="margin-left:5px;font-size:14px;">{{item.deptSubName}}</span></div>
			</div>
